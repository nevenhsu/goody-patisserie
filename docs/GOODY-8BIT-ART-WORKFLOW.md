...
6. 生成前檢查 `public/imagegen/asset-spec.json` 與 [`GOODY-ASSET-CLASSES.md`](GOODY-ASSET-CLASSES.md)。每個 asset 都必須指定已註冊的 class 與 spec version，不得發明 one-off dimensions。目前 schema v9 portrait structural assets 使用 `scene-portrait-ceiling-v2`、`scene-portrait-backwall-v2`，並以 `scene-projective-wall`、`scene-projective-floor-16x8` canonical flat textures 做 runtime projection；active counter 使用 `counter-base-portrait-v3`、`counter-top-portrait-v3`。舊 classes 與 manifest entries 只保留 immutable delivery history。
